...
public class MathUtil {
  ...
  public double[] trimr(double[] arr, int n1, int n2) {
    checkArgument(n1 + n2 < arr.length, "Attempting to trim too much in trimr");

    int h2 = arr.length - n2;
  ...
  public RealMatrix trimr(RealMatrix m, int n1, int n2) {
    checkArgument(n1 + n2 < m.getRowDimension(), "Attempting to trim too much in trimr");

    int h2 = m.getRowDimension() - 1 - n2;

    return m.getSubMatrix(n1, h2, 0, m.getColumnDimension() - 1);
  }


  public double[] tdiff(double[] arr, int k) {
    if(k == 0) {
      return arr;
    }
  ...
  public RealMatrix tdiff(RealMatrix m, int k) {
    if(k == 0) {
      return m;
    }
    int rSize = m.getRowDimension();
    int cSize = m.getColumnDimension();
    RealMatrix newMatrix = new Array2DRowRealMatrix(rSize, cSize);
    for(int i = rSize - 1; i >= k; i--) {
  ...
  public double[][] pTrend(int p, int nobs) {

    double[][] arr = new double[nobs][p + 1];
    ...
    if(p > 0) {
      for(int i = 0; i < nobs; i++){
        ...
        for(int j = 1; j < p + 1; j++) {
          if(j == 1){
            arr[i][j] = (double)(i + 1) / nobs;
          } else {
            arr[i][j] = Math.pow(arr[i][1], j);
          }
        }
      }
   ...
   */
  public double[] lag(double[] arr, int n, int v) {
    if(n == 0) {
      return arr;
    }
    double[] newArr = new double[arr.length];
  ...
  public RealMatrix lag(RealMatrix m, int n, int v) {
    if(n == 0) {
      return m;
    }
    RealMatrix newMatrix = new Array2DRowRealMatrix(m.getRowDimension(), m.getColumnDimension());
    for(int i = m.getRowDimension() - 1; i >= 0; i--) {
      for(int j = 0; j < m.getColumnDimension(); j++) {
        if(i < n) {
          newMatrix.setEntry(i, j, v);
        } else {
  ...
  public double[][] prependArrayAsColumn(double[] arr, double[][] twoDArr) {
    checkArgument(arr.length == twoDArr.length, "array sizes are not equal");

    for(int i = 0; i < twoDArr.length; i++) {
      double[] newRow = new double[twoDArr[i].length + 1];
      newRow[0] = arr[i];
      System.arraycopy(twoDArr[i], 0, newRow, 1, twoDArr[i].length);
      twoDArr[i] = newRow;
    }
    return twoDArr;
  }

  public double[][] prependArrayAsColumn(double[] arr, double[] arr2) {
    checkArgument(arr.length == arr2.length, "array sizes are not equal");
    double[][] newArr = new double[arr.length][];
    for(int i = 0; i < arr.length; i++) {
      newArr[i] = new double[]{arr[i], arr2[i]};
    }
    return newArr;
  }


  public double[] zCrit(int nobs, int p) {
    checkArgument(nobs >= 50, "nobs can not be less than 50");
    ...
    int i = (int)Math.round((double)nobs/50) + 1;
    if (nobs < 50) {
      i--;
    }
    if(i > 10) {
  ...
  public double[] rztcrit(int nobs, int k, int p) {
  ...
  public double[][] detrend(double[] timeSeries, int p) {
    if(p == -1) {
      return MatrixUtils.createColumnRealMatrix(timeSeries).getData();
    }

    double[] u = ones(timeSeries.length);

    if(p > 0) {
      throw new UnsupportedOperationException("only p 0 and -1 supported");
    } else {

      RealMatrix xMat = MatrixUtils.createColumnRealMatrix(u);
      RealMatrix yMat = MatrixUtils.createColumnRealMatrix(timeSeries);
      RealMatrix xpxi = MatrixUtils.inverse(xMat.transpose().multiply(xMat));
      RealMatrix beta = xpxi.multiply(xMat.transpose().multiply(yMat));
      return yMat.subtract(xMat.multiply(beta)).getData();
    }
  }

  public RealMatrix detrend(RealMatrix m, int p) {
    if(p == -1) {
      return m;
    }

    double[] u = ones(m.getRowDimension());
    ...
    if(p > 0) {
      ...
      RealMatrix xMat = MatrixUtils.createColumnRealMatrix(u);
      RealMatrix xpxi = MatrixUtils.inverse(xMat.transpose().multiply(xMat));
      RealMatrix beta = xpxi.multiply(xMat.transpose().multiply(m));
      return m.subtract(xMat.multiply(beta));
    }
  }

  public double[] ones(int arrSize) {
    double[] onesArray = new double[arrSize];
